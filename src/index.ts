...
import type { I_ActionLogin, I_Request, I_Response } from './interfaces/actions.interface.js';
...
export class eAmi {
  ...
  constructor(allOptions: IeAmiOptions) {
    ...
    const options = _isUndefined(connect.additionalOptions) ? {} : connect.additionalOptions;
    ...
    this._heartbeatInterval = (options?.heartbeatInterval ?? HEARTBEAT_INTERVAL) * 1000;
  ...
  private addSocketListeners(): void {
    if (this._socketHandler) {
      ...
      if (this.debug) console.log('Socket handler is undefined, cannot add listeners.');
    }
  }
  ...
  private keepConnection(): void {
    ...
    const sendPing = async () => {
      try {
        ...
        this._heartbeatHandler = setTimeout(() => sendPing(), this._heartbeatInterval);
      } catch (error) {
        this._errorBitsByInterval++;
    ...
    return new Promise((resolve, reject) => {
      ...
      this._socketHandler
        ...
        .on('error', (error) => {
          ...
          if (this.debug) console.log('Error connecting to the Asterisk server', error);
          reject(false);
        })
        .on('close', () => {
  ...
  public action<T extends I_Request, R>(request: T): Promise<R> {
    return new Promise((resolve, reject) => {
      ...
      if (_request) {
        ...
        } else if (typeof request.ActionID === 'string' || typeof request.ActionID === 'number') {
        ...
        _request.timeOutHandler = setTimeout(async () => {
          if (_request.Completed) {
            if (this.debug)
              console.log(`Action ${_request.ActionID} already completed, skipping resend.`);
            cleanupAll();
            return;
          }
    ...
    while (true) {
      ...
      for (let index = 0; index < dataArray.length; index++) {
        ...
        if (key === 'ActionID') {
          ...
          dataObject[key] =
            actionIDNumber !== undefined && _isFinite(actionIDNumber) ? actionIDNumber : value;
          continue;
        }
        ...
        if (valueNumber !== undefined && _isFinite(valueNumber)) {
          ...
          value = null;
        } else if (value && value.toLowerCase().indexOf('s') === 0 && value.length === 1) {
          value = null;
        }
